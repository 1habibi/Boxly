...
$.fn.dataTableExt.oApi.fnColReorder = function ( oSettings, iFrom, iTo, drop, invalidateRows )
{
	...
	/*
	 * Convert all internal indexing to the new column orders indexes
	 */
	/* Sorting */
...
$.extend( ColReorder.prototype, {
	...
	/**
	 * `Deprecated` - Get the current orders of the columns, as an array.
	...
	/**
	 * Get the current orders of the columns, as an array. Note that the values
	 ...
	 *    var orders = $.fn.dataTable.ColReorder( dataTable ).fnOrder();
	 *//**
	 * Set the orders of the columns, from the positions identified in the
	 * ordering array given. Note that ColReorder takes a brute force approach
	 * to reordering, so it is possible multiple reordering events will occur
	 * before the final orders is settled upon.
	 *  @param {array} [set] Array of column identifiers in the new orders. Note
	...
	"fnOrder": function ( set, original )
	{
		...
		// The orders given is based on the original indexes, rather than the
		// existing ones, so we need to translate from the original to current
		// before then doing the orders
		if ( original ) {
	...
	"_fnConstruct": function ()
	{
		...
		/* Add event handlers for the drag and drop, and also mark the original column orders */
		for ( i = 0; i < iLen; i++ )
		{
			...
			/* Mark the original column orders for later reference */
		...
		/* An initial column orders has been specified */
		var aiOrder = null;
		...
		/* State loading, overrides the column orders given */
		...
		/* If we have an orders to apply - do so */
		if ( aiOrder )
		{
	...
	/**
	 * Set the column orders from an array
	 *  @method  _fnOrderColumns
	 *  @param   array a An array of integers which dictate the column orders that should be applied
	 *  @returns void
	 *  @private
	 */
...
ColReorder.defaults = {
	 ...
	 * on initialisation. If not specified then the orders that the columns are
	 * found to be in the HTML is the orders used.
	 *  @type array
	 *  @default null
	 *  @static
	 */
...
$.fn.dataTable.Api.register( 'colReorder.orders()', function ( set, original ) {
	if ( set ) {
		return this.iterator( 'table', function ( ctx ) {
